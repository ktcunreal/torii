...
type SnappyStreamServer struct {
	net.Conn
	w *snappy.Writer
}

func NewSnappyStreamServer(conn net.Conn) *SnappyStreamServer {
	s := &SnappyStreamServer{Conn: conn}
	s.w = snappy.NewBufferedWriter(conn)
	return s
}

func (s *SnappyStreamServer) Read(b []byte) (n int, err error) {
	return s.Conn.Read(b)
}

func (s *SnappyStreamServer) Write(b []byte) (n int, err error) {
...
func (s *SnappyStreamServer) Close() error {
	return s.Conn.Close()
}
